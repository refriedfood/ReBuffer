    ...
    return {
      storage: {
        ...
        session: chrome.storage && chrome.storage.session ? {
          get: promisify(chrome.storage.session, "get"),
          set: promisify(chrome.storage.session, "set")
        } : undefined
      },
      tabs: {
...
const autoplayTabs = {}; // in-memory fallback (MV2)

// MV3 service worker note: in-memory state is not reliable because the worker can be suspended.
// We'll mirror autoplay flags into chrome.storage.session when available (falls back to storage.local).
async function _autoplayStoreGet() {
  try {
    if (rb.storage && rb.storage.session && typeof rb.storage.session.get === "function") {
      const r = await rb.storage.session.get("autoplayTabs");
      return r.autoplayTabs || {};
    }
  } catch (e) {}

  try {
    const r2 = await rb.storage.local.get("autoplayTabs");
    return r2.autoplayTabs || {};
  } catch (e2) {}

  return {};
}

async function _autoplayStoreSet(obj) {
  try {
    if (rb.storage && rb.storage.session && typeof rb.storage.session.set === "function") {
      await rb.storage.session.set({ autoplayTabs: obj });
      return;
    }
  } catch (e) {}

  try {
    await rb.storage.local.set({ autoplayTabs: obj });
  } catch (e2) {}
}

async function _autoplayStoreMark(tabId) {
  if (tabId == null) return;
  const obj = await _autoplayStoreGet();
  obj[String(tabId)] = Date.now();
  await _autoplayStoreSet(obj);
}

async function _autoplayStoreConsume(tabId) {
  ...
  (async function () {
    try {
      ...
      if (message.type === "rebuffer_getConfig") {
        ...
        var did = false;
        try {
          // MV3: consume from persisted store
          did = await _autoplayStoreConsume(tabId);
        } catch (e) {}
        if (!did && autoplayTabs[tabId]) {
          // MV2 fallback
          did = true;
      ...
      try {
        await _autoplayStoreMark(tabId);
      } catch (e) {}
...
function contextsForMenus(globalCfg) {
  // Chrome MV2 here: valid contexts include "page" and "action". No "tab".
