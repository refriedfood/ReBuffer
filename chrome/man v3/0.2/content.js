...
VideoTracker.prototype.resetPiP = function () {
  ...
  // If we update lastProgressTs without real playback progress, PiP hang detection can stop triggering,
...
function isVideoInPiP(video) {
  ...
  // Fallback: CSS pseudo-class supported by Chromium-based browsers
  try {
    if (video.matches && video.matches(":picture-in-picture")) {
      return true;
    }
  } catch (e1) {}

  // Some environments expose webkitPresentationMode
  try {
    if (typeof video.webkitPresentationMode === "string" && video.webkitPresentationMode === "picture-in-picture") {
      return true;
    }
  } catch (e2) {}
...
function tick() {
  ...
  for (var i = 0; i < videos.length; i++) {
    ...
    rb.runtime
      .sendMessage({ type: "rebuffer_hung" })
      .catch(function () {});
